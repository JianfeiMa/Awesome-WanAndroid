...
public class MainPagerFragment extends BaseRootFragment<MainPagerPresenter> implements MainPagerContract.View {
    ...
    SmartRefreshLayout mRefreshLayout;//智能刷新布局，下拉刷新，上拉加载
    @BindView(R.id.main_pager_recycler_view)
    RecyclerView mRecyclerView;//循环器
    ...
    /**
     * 重写BaseRootFragment中的方法，从服务器中获取需要展示的数据
     */
    @Override
    protected void initEventAndData() {
        ...
        if (loggedAndNotRebuilt()) {
            mPresenter.loadMainPagerData();//从这里开发执行程序从服务器中获取数据
        } else {
